...
pub fn new_full(config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    let mut net_config: FullNetworkConfiguration<Block, <Block as sp_runtime::traits::Block>::Hash, Litep2pNetworkBackend> =
        FullNetworkConfiguration::new(&config.network);

    let genesis_hash: <Block as sp_runtime::traits::Block>::Hash = client
        .block_hash(0u32)
        .ok()
        .flatten()
