...
#[derive(Parser)]
struct Options {
	/// The path to the `xnetx-node` source.
	#[arg()]
...
/// Copy the `xnetx-node` to the given path.
fn copy_node_template(node_template: &Path, dest_path: &Path) {
	let options = CopyOptions::new();
	dir::copy(node_template, dest_path, &options).expect("Copies xnetx-node to tmp dir");
}
...
/// Build and test the generated xnetx-node
...
fn main() {
	...
	// Copy xnetx-node to a temp build dir.
	...
	// The path to the xnetx-node in the build dir.
	...
	// Get all `Cargo.toml` files in the xnetx-node
	...
	tar.append_dir_all("substrate-xnetx-node", node_template_path)
		.expect("Writes substrate-xnetx-node archive");
}
